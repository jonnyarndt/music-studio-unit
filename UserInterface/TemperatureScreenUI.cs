...
namespace musicStudioUnit.UserInterface
{
    ...
    public class TemperatureScreenUI : IDisposable
    {
                                  ...
                                  MSUController msuController, List<TemperaturePreset> presets = null)
        {
            ...
            PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Initializing Temperature screen UI");
            ...
            PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature screen UI initialized successfully");
        }
        ...
        private void SetupTouchPanelEvents()
        {
            ...
            PepperDash.Core.Core.Debug.Console(2, "TemperatureScreenUI", "Touch panel events configured");
        }
        ...
        private void OnTemperatureUpPressed()
        {
            lock (_lockObject)
            {
                try
                {
                    ...
                    if (newTemp > 50.0f)
                    {
                        PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature {0}°C exceeds maximum (50°C)", newTemp);
                        ShowTemperatureError("Maximum temperature reached");
                        return;
                    }

                    PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature UP: {0:F1}°C -> {1:F1}°C", 
                ...
                {
                    PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error handling temperature up: {0}", ex.Message);
                }
            }
        }
        ...
        private void OnTemperatureDownPressed()
        {
            lock (_lockObject)
            {
                try
                {
                    ...
                    if (newTemp < -40.0f)
                    {
                        PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature {0}°C below minimum (-40°C)", newTemp);
                        ShowTemperatureError("Minimum temperature reached");
                        return;
                    }

                    PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature DOWN: {0:F1}°C -> {1:F1}°C", 
                        _currentDisplayTemp, newTemp);
                ...
                {
                    PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error handling temperature down: {0}", ex.Message);
                }
            }
        }
            ...
            lock (_lockObject)
            {
                try
                {
                    if (presetIndex >= 0 && presetIndex < _presets.Count)
                    {
                        ...
                        PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Preset '{0}' selected: {1:F1}°C", 
                ...
                {
                    PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error handling preset button: {0}", ex.Message);
                }
            }
        }
        ...
        private void SetTemperature(float temperature)
        {
            try
            {
                ...
                {
                    PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Cannot set temperature - HVAC not connected");
                ...
                if (success)
                {
                    ...
                    PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature set successfully: {0:F1}°C for zones: {1}", 
                        temperature, string.Join(",", _controlledZones));
                }
                else
                {
                    PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Failed to set temperature");
                    ShowTemperatureError("Failed to set temperature");
                }
            }
            catch (Exception ex)
            {
                PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error setting temperature: {0}", ex.Message);
        ...
        private void UpdateControlledZones()
        {
            try
            {
                ...
                PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Controlled zones updated: {0}", 
            ...
            {
                PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error updating controlled zones: {0}", ex.Message);
                _controlledZones.Clear();
        ...
        private void UpdateZoneDisplay()
        {
            ...
            {
                PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error updating zone display: {0}", ex.Message);
            }
        }
        ...
        private void LoadSavedSetpoints()
        {
            try
            {
                ...
                foreach (var zoneId in _controlledZones)
                {
                    ...
                    if (!string.IsNullOrEmpty(savedValue) && float.TryParse(savedValue, out float setpoint))
                    {
                        ...
                        PepperDash.Core.Core.Debug.Console(2, "TemperatureScreenUI", "Loaded saved setpoint for zone {0}: {1:F1}°C", 
                            zoneId, setpoint);
                    }
                    else
                    {
                        ...
                        PepperDash.Core.Core.Debug.Console(2, "TemperatureScreenUI", "Using idle setpoint for zone {0}: {1:F1}°C", 
                            zoneId, idleSetpoint);
                    }
                }
            ...
            {
                PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error loading saved setpoints: {0}", ex.Message);
            }
        }
        ...
        private void SaveSetpointToNonvolatile(float temperature)
        {
            try
            {
                foreach (var zoneId in _controlledZones)
                {
                    ...
                    PepperDash.Core.Core.Debug.Console(2, "TemperatureScreenUI", "Saved setpoint for zone {0}: {1:F1}°C", 
                        zoneId, temperature);
                }
            }
            catch (Exception ex)
            {
                PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error saving setpoint to nonvolatile storage: {0}", ex.Message);
            }
        }
        ...
        private void UpdateUI()
        {
            lock (_lockObject)
            {
                ...
                try
                {
                    ...
                    PepperDash.Core.Core.Debug.Console(2, "TemperatureScreenUI", "UI updated successfully");
                }
                catch (Exception ex)
                {
                    PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error updating UI: {0}", ex.Message);
                }
                finally
                {
                    _isUpdatingUI = false;
        ...
        private void UpdateTemperatureDisplay()
        {
            ...
            PepperDash.Core.Core.Debug.Console(2, "TemperatureScreenUI", "Temperature display updated: {0}", tempText);
        }
        ...
        private void ShowTemperatureError(string message)
        {
            ...
            PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature error: {0}", message);
        }
        ...
        {
            PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "HVAC connected - updating UI");
            UpdateUI();
        }

        private void OnHVACDisconnected(object sender, HVACDisconnectedEventArgs args)
        {
            PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "HVAC disconnected - updating UI");
            UpdateUI();
        }

        private void OnHVACError(object sender, HVACErrorEventArgs args)
        {
            PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "HVAC error: {0}", args.ErrorMessage);
            ShowTemperatureError(args.ErrorMessage);
        }

        private void OnCombinationChanged(object sender, CombinationChangedEventArgs args)
        {
            PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Combination changed - updating controlled zones");
        ...
        public void Dispose()
        {
            ...
            try
            {
                ...
                PepperDash.Core.Core.Debug.Console(1, "TemperatureScreenUI", "Temperature screen UI disposed");
            }
            catch (Exception ex)
            {
                PepperDash.Core.Core.Debug.Console(0, "TemperatureScreenUI", "Error disposing: {0}", ex.Message);
            }
            finally
            {
                _disposed = true;
